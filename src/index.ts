  ...
  async function generateRankingChart({
    ...
    try {
      ...
      await Promise.all(
        data.map(async (item) => {
          try {
            const response = await ctx.http.get(item.avatar, {
            ...
            item.avatarBase64 = (await canvas.toBuffer("image/png")).toString(
              "base64"
            );
          } catch (e) {
            logger.warn(
              `获取或处理头像失败 (${item.avatar})，将使用默认头像: ${e.message}`
            );
            // 提供一个50x50的灰色方块作为备用头像
            item.avatarBase64 =
              "iVBORw0KGgoAAAANSUhEUgAAADIAAAAyCAYAAAAeP4ixAAAAAXNSR0IArs4c6QAAADhJREFUaEPt0AENAAAMwqD3T22IL43g4R331IK7gAABAgQIECBAgAABAgQIECBAgAABAgQIECBAgMDLBSs2aAP6BAAAAABJRU5ErkJggg==";
          }
        })
      );
